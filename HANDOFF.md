...
**THE finding:** every lever that changed the article FAILED to beat V1-alone. On task 57 (rich evidence, mostly-replayed cache): baseline (no window) 35.20 → **+V1 exposure 37.82** (also grew length 21k→30k) → +depth (48 spans + 28-sentence writer) **33.28** (shorter, 26k, −4.5). So: (a) V3 pre-write notes = flat; (b) V7 per-entity restructure = −4.9; (c) **DEPTH TESTED + REJECTED** — more spans + a higher writer cap make the writer *compress* (shorter, worse), it does NOT expand toward the 83k reference. **The one lever that helps is V1's wider CONTEXT-per-cited-span (exposure), NOT more spans.** The prose is already analytical/cross-source → insight is bounded by the **local writer's (mistral-small3.2) capacity**, not the scaffold or the evidence volume. **The ~3-6 RACE gap to 40.67 is a MODEL problem.** (Caveat: task-53 V8/depth cells were INERT — span-poor frozen pages, caps didn't bind, identical article, judge-noise only; the real depth test is the task-57 one above.)
...
1. **The lever avenue is exhausted; do NOT keep building scaffolds expecting >35.** Depth is now TESTED + rejected (task 57: V1-alone 37.82 > V1+depth 33.28). Across v7 (W1-W5), v9 (R1-R6), v10 (V1/V3/V7/V8/depth), only **V1 exposure** + the react+scope+WARP `vague` stack are net-positive; everything else washes, hurts, or is inert on this writer. Cheap remaining validation, NOT new levers: re-judge each saved frozen article ×3-5 to nail the true means (judge noise ≈ ±1) — the near-noise deltas (V1 +2.6, V7 −4.9) deserve it.
