...
**Commits (feat/deepresearch-bench, PUSHED to origin `4964a1b..d54336f`; all default-off, byte-identical default path, 750 unit green, ruff+mypy clean):**
`a38b6fe` V1 spans · `78bcc9c` vague profile + V3b tables · `9e43cd6` bench A/B harness + OLLAMA_HOST + Tongyi tags · `0734ca2` V3 notes (opt-in, wash) · `30d9144` V7 entity rubric (opt-in, negative) · `d1d3ab2` env REACT_MAX_ITERS · `c786433` V8 spans-per-page (default-off; inert on task-53 thin evidence) · `5d10cfa` promote V1 into vague profile · `8b7c384` docs + A/B harness scripts · `2ce4901` correct V8 verdict (deterministic engine, judge noise) · `d54336f` depth tested + rejected on task-57.
