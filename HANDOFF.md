...
**One-line state:** autonomous session committed the prior session's validated V1 win, promoted V1 into the `vague` preset, and built + measured 3 new levers. **Result: V1 = real win (banked); V3 (cross-source notes) = wash; V7 (entity structure) = negative; V8 + depth-stack = INCONCLUSIVE — the span/writer-length flags did NOT bind on task-53's span-poor frozen evidence (engine produced a byte-identical article). Inference-time SCAFFOLD levers (V3 notes, V7 structure) don't lift RACE on the local writer; the evidence-DEPTH hypothesis remains untested.** 8 commits on `feat/deepresearch-bench` (NOT pushed). Infra UP (SearXNG :8080, Ollama, bridge :11444).

**⚠️ METHODOLOGY CORRECTION (verified this session — overrides the prior "writer non-determinism" heuristic):** the engine is **DETERMINISTIC on the frozen cache** (temp=0) — all 3 reps of every config produced **byte-identical articles** (`v1 r1==r2==r3`; `v8 all==`; and `v8==v1` exactly). So the run-to-run RACE spread (e.g. V1 reps 34.81/35.80/33.35 on ONE identical article) is **JUDGE (kimi) noise, ~±1 sd / up to ~2.5 range — NOT writer noise**. Consequences: (a) any single-judge RACE delta < ~2.5 is noise; (b) correct protocol = run each config's article ONCE (deterministic), **judge it N times** to average judge noise — do NOT re-run the engine N times; (c) a lever only "counts" if it actually changes the article (verify with a diff, as done here).

**Measured this session (kimi judge via bridge; task-53 cells = frozen-cache, deterministic article):**
| lever | test | article changed? | RACE | verdict |
|---|---|---|---|---|
| **V1 spanwin** | task-57 class-proof (baseline 35.20 → +V1) | yes (wider spans) | **37.82** (+2.6, insight +5.9); task-53 FACT +5.4pt | ✅ real win — class-proven, PROMOTED into `vague` |
| V3 notes | task-53 frozen (V1 vs V1+V3) | yes (+1385 chars, added section) | 34.65→34.98 (+0.33) | ⚠️ WASH — real change, RACE flat (inside judge noise) |
| V7 entity | task-57 N=1 (thematic+V1 vs entity+V1) | yes (per-firm restructure) | 37.82→**32.90** (−4.9) | ❌ NEGATIVE — structure w/o depth fragments (5/13 firms) |
| V8 spans32 | task-53 frozen (V1@20 vs @32) | **NO — byte-identical to V1** | 34.65 vs 33.53 = judge noise | ⛔ INERT — task-53 pages span-poor, cap didn't bind |
| depthstack | V8 + writer 28/3600, task-53 frozen | **NO — byte-identical to V1** | judge noise | ⛔ INERT — thin evidence, writer cap didn't bind |

**THE finding (corrected):** RACE does NOT move with a pre-write synthesis pass (V3 — real article change, flat RACE) or per-entity restructuring (V7 — negative). The article prose is **already analytical and cross-source** ("the consulting pyramid is starting to crack") → insight is bounded by the **local writer's (mistral-small3.2) analytical capacity**, not the scaffold. **The evidence-DEPTH hypothesis is UNTESTED** — V8/depth were inert because task-53's frozen evidence is span-poor (the known task-53 thin-fetchability problem), so the span/writer caps never bind and the article is fixed. **To actually test depth: run V8/higher-writer-cap on the RICH-evidence task-57 FROZEN cache** (flags bind AND the article stays deterministic → one engine run + N judge samples). Diagnostic stands: task-57 = 21k/7 thematic vs reference 83k/13 per-entity — the gap is a stronger writer + more evidence, which restructuring alone can't fake.
...
`a38b6fe` V1 spans · `78bcc9c` vague profile + V3b tables · `9e43cd6` bench A/B harness + OLLAMA_HOST + Tongyi tags · `0734ca2` V3 notes (opt-in, wash) · `30d9144` V7 entity rubric (opt-in, negative) · `d1d3ab2` env REACT_MAX_ITERS · `c786433` V8 spans-per-page (default-off; inert on task-53 thin evidence, depth untested) · `5d10cfa` promote V1 into vague profile · `8b7c384` docs + A/B harness scripts.

**NEXT SESSION — the honest fork:**
1. **Two clean next experiments before any big build:** (a) **test the depth hypothesis properly** — V8 (`MAX_PAGE_SPANS=48`) + `WRITER_MAX_SENTENCES=28` on the **task-57 FROZEN cache** (rich evidence → the flags actually bind; deterministic article → 1 engine run, judge ×N). This is the one lever V8 failed to test on span-poor task 53. (b) **re-confirm the near-noise wins** (V1 +2.6, V7 −4.9) by judging each frozen article ×3-5 (judge noise is the real ±1 variance). Do NOT keep building new *scaffold* levers expecting >35 — across v7 (W1-W5), v9 (R1-R6), v10 (V1/V3/V7), only V1 + the react+scope+WARP `vague` stack are net-positive; the rest wash or hurt on this writer.
